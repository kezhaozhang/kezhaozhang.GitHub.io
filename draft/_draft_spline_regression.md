...
**he B-spline basis system has a property that is often useful: the sum of the B-spline basis function values at any point t is equal to one.** Note, for example, in Figure 3.1 that the ﬁrst and last basis functions are exactly one at the boundaries

The methods of B-splines provide general procedures for defining these functions. **The term B-spline specifically refers to one of the basis functions, not the function created by the linear combination of a set of B-splines.** However, there is inconsistency in how the term is used in computer graphics.

...regression-like equations and, consequently, all the ordinary least squares (OLS) theory, including the very useful leave-oneout cross-validation equations, can be applied. **The idea in B-splines is the use of an orthogonal basis of functions, the B-spline basis.** The idea is analogous to Fourier series or to wavelet



Properties of basis functions

https://pages.mtu.edu/~shene/COURSES/cs3621/NOTES/spline/bspline-property.html
...
$\alpha$ is solved by minimizing $|y-B\alpha|^2=(y-B\alpha)^T(y-B\alpha)$. 
...
We generate some one-dimensional data and use both P-splines and Bayesian regression to fit the data.

### Data

First, let us generate some data, which are plotted in Figure 3.
```python
import numpy as np
...
def spline_fit(X, Y, n_knots, degree, lam, x_new):
    ...
    Output:
        y_new: predicted value of y for x_new
    """
    from sklearn.preprocessing import SplineTransformer
...
Figure 5 shows the data, spline fit, B-spline basis functions, and fit coefficients in the same plot. The spline fit (black line) in the sum of the weighted basis functions (colored lines).
...
$$
y = B\alpha +\mathcal{N}(0, \sigma_y), \label{eqn:ols}
$$
where $B$ is a matrix representing the values of the spline bases at the data points, $x$. 

To restrict the difference among the coefficients $\alpha$, we set the components of $\alpha$ to be from the Gaussian random walk so that they are correlated. The variance of the random walk, $\sigma$ is controlled by its prior distribution--an exponential distribution in this study. A single parameter λ controls the exponential distribution whose  PDF is 
$$
\lambda \exp(-\lambda x). \notag
$$
